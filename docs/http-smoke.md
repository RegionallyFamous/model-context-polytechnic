...
9. Verify `get-exercise` exposes `rubric_vocabulary.required_terms`.
...
11. `tools/call` for `get-learning-memory`.
12. `tools/call` for `get-certificate` and confirm an unfinished enrollment gets remaining work.
13. `tools/call` for `get-exercise` with `include_model_answer=true`.
